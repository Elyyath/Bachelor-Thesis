...
class Agent():
    def __init__(self, strategy, n_actions, gamma, lr, img_height, img_width, batch_size, chkpt_dir='tmp'):
        self.current_step = 0
    ...
    def select_action(self, state):
        ...
        if self.rate > random.random():
            ...
            with torch.no_grad():
                ...
                return actions.argmax(dim=1).to(self.policy_net.device) # exploit
    ...
    #Methode, zur Durchführung des DQN Lernalgorithmus    
    def learn(self):
        if self.memory.can_provide_sample(self.batch_size):
            ...
            q_eval = self.policy_net.forward(states)[indices, actions] #Q-Werte der gewählten Aktionen in den Zuständen s_t
            q_next = self.target_net.forward(next_states)# Q-Werte für alle Aktionen in den nächsten Zuständen s_t+1
            q_next[dones] = 0.0
            q_target = rewards + self.gamma * torch.max(q_next, dim=1)[0] #Berechnung der Target Q-Werte
...
if __name__ == '__main__':
    ...
    with open('data/rewards_all_episodes01_dqn.json', 'w', encoding='utf-8') as f:
